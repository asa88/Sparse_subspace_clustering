...
X = [X1 X2];
s = [1*ones(1,N1) 2*ones(1,N2)]; %Generaipython profile createting the ground-truth for evaluating clustering results
...
parfor i=1:size(X,2),
	...
	c = SparseCoefRecovery(Xp,Cst,OptM,lambda,i);
 	if i == 1   
	   ...
	   CMat(:,i)=temp;
	elseif ( (i > 1) && (i < N) )
        temp(1:i-1,1) = c(1:i-1);
        temp(i+1:N,1) = c(i:N-1);
		CMat(:,i)= temp
    else
        temp(1:N-1,1) = c(1:N-1);
		CMat(:,i)=temp
    end
